...
local SemanticPatterns = {
    ...
    {
        Keywords = {"esp", "box", "skeleton", "health", "name", "item", "chest", "ore", "npc", "tracer", "cham", "xray", "radar", "minimap", "distance", "fov", "fullbright", "vision", "fog", "crosshair"},
...
print("PayomboyZ AI: Visual Matrix and ESP Highlights Enabled.")]]
    ...
    {
        Keywords = {"farm", "collect", "sell", "buy", "upgrade", "quest", "spin", "roll", "fish", "mine", "chop", "harvest", "hatch", "craft", "rebirth", "prestige", "claim", "open", "pickup", "deposit"},
...
print("PayomboyZ AI: Automation & Framework Cycle Enabled.")]]
    ...
    {
        Keywords = {"god", "afk", "ragdoll", "stun", "knockback", "fling", "void", "stamina", "energy", "oxygen", "fall", "respawn", "revive", "freeze", "unfreeze", "invisible", "lock", "freecam", "spectate", "fps"},
...
print("PayomboyZ AI: God-State & Anti-Debuff Framework Initialized.")]]
    ...
    -- 4. MOVEMENT CATEGORY
    {
        Keywords = {"fly", "noclip", "speed", "sprint", "jump", "climb", "run", "dash", "slide", "teleport", "blink", "glide", "hover", "swim", "gravity", "hop", "parkour", "phase"},
...
print("PayomboyZ AI: Kinematic Movement speed & Noclip modules deployed.")]]
    ...
    {
        Keywords = {"hub", "menu", "admin", "spy", "remote", "server", "hop", "rejoin", "id", "bring", "save", "waypoint", "notification", "key", "config", "macro", "clicker", "lag", "booster", "player", "emote", "skin", "pet", "inventory"},
...
print("PayomboyZ AI: Miscellaneous Matrix and Remote Interceptors Online.")]]
    ...
    -- 6. COMBAT / AIM ASSIST
    {
        Keywords = {"aim", "lock", "hitbox", "size", "extend", "killaur", "aura", "attack"},
...
promptLabel.Text = "ENTER SEMANTIC INTENT PROMPT"
...
PromptInput.PlaceholderText = "e.g. 'katana', 'plasma cannon', 'esp', 'fly', 'farm'"
PromptInput.Text = ""
...
-- Process Button
local processBtnObj = AiTab:AddButton({
    Title = "⚡ Formulate Lexical Output Code",
    Callback = function() end
})
...
local CodeTerminal = Instance.new("TextBox")
...
-- Save Button
local saveBtnObj = AiTab:AddButton({
    Title = "💾 Save Compiled Assets to Scroll Library",
    Callback = function() end
})
...
processBtnObj.MouseButton1Click:Connect(function()
    ...
    local text = string.lower(userPrompt)
    if text == "" then
        ObsidianGlassEngine:Notify({ Title = "Warning", Content = "Please enter a prompt first!", Duration = 3 })
        return
    end

    DebugLog("INFO", "AI_SYNTH", "Synthesizing code for prompt: '" .. userPrompt .. "'")
    ...
    if #AIContextEngine.BoundItems > 0 then
        for i, item in ipairs(AIContextEngine.BoundItems) do
            codeLines[#codeLines + 1] = string.format("local Bound_%d = %s -- %s", i, item.FullName, item.ClassName)
        end
    ...
    if #matchedBlocks > 0 then
        for _, block in ipairs(matchedBlocks) do
            codeLines[#codeLines + 1] = "-- [[ SUB-SYSTEM: " .. block.Title .. " ]]"
            if block.Title == "Weapon_Tool_Synthesizer" then
                codeLines[#codeLines + 1] = "local PromptInput = {Text = [[" .. userPrompt .. "]]}\n" .. block.Code
            else
                codeLines[#codeLines + 1] = block.Code
            end
            codeLines[#codeLines + 1] = ""
        end
    else
        codeLines[#codeLines + 1] = "-- [[ GENERIC STRUCTURAL TEMPLATE ]]\nprint(\"PayomboyZ AI V6: Executing contextual module for prompt: \" .. " .. string.format("%q", userPrompt) .. ")"
    end
...
local function populateLibraryScroll()
...
saveBtnObj.MouseButton1Click:Connect(function()
    local code = CodeTerminal.Text
    if code ~= "" then
        local moduleName = "Synthesized_Asset_" .. tostring(MacroCounter)
        table.insert(SessionRegistry.Macros, {Name = moduleName, Code = code})
        MacroCounter = MacroCounter + 1
        CodeTerminal.Text = ""
        populateLibraryScroll()
        ObsidianGlassEngine:Notify({ Title = "Saved", Content = "Module saved to Command Matrix!", Duration = 3 })
    else
        ObsidianGlassEngine:Notify({ Title = "Warning", Content = "Code terminal is empty!", Duration = 3 })
    end
end)
...
SettingsTab:AddSection("CONTROL_SECTION", "❌ SYSTEM CONTROL")
